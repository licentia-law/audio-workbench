...
const PageHeader = () => (
  <div className="flex items-start justify-between mb-6">
    <div>
      <h1 className="text-[28px] font-semibold tracking-tight">음원 자르기</h1>
      <p className="text-fg-dim text-[14px] mt-1.5">원하는 구간만 선택하여 잘라낼 수 있습니다.</p>
    </div>
...
// Upload + file meta card (combined header card)
const UploadFileCard = ({ state, file }) => {
  ...
  return (
    <div className="rounded-2xl border border-line bg-ink-700 shadow-card p-5">
      <div className="grid" style={{ gridTemplateColumns: '380px 1fr' }}>
        {/* Drop zone */}
        <div className={`relative rounded-xl border ${uploaded ? 'border-line2 border-dashed' : 'border-dashed border-brand-cyan/40 bg-brand-cyan/5'} px-5 py-6 flex flex-col items-center justify-center text-center min-h-[140px]`}>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2.5">
                    ...
                    <Badge tone="ok"><Icon name="check" className="w-3 h-3" />업로드 완료</Badge>
                  </div>
                  <div className="text-[12px] text-fg-mute mt-0.5">방금 업로드됨</div>
                </div>
                <button className="w-9 h-9 grid place-items-center rounded-md border border-line2 text-fg-mute hover:text-err hover:border-err/50">
              ...
              <div className="grid grid-cols-5 gap-3">
                {[
                  ...
                  <div key={k} className="rounded-lg bg-ink-800 border border-line2/50 px-3 py-2">
                    ...
                    <div className="text-[14px] font-medium num mt-0.5">{v}</div>
                  </div>
                ))}
              </div>
            </div>
...
// Selection info card (start / end / length)
const SelectionInfo = ({ start, end, total }) => {
  const len = Math.max(0, end - start);
  return (
    <div className="rounded-2xl border border-line bg-ink-700 shadow-card grid grid-cols-3 divide-x divide-line2/40 mt-5">
      {[
        { label: '시작 시점', big: fmt(start, true), sub: '범위 0:00.00', tone: 'cyan' },
        { label: '종료 시점', big: fmt(end, true),   sub: `범위 ${fmt(total, true)}`, tone: 'indigo' },
        { label: '선택 길이', big: fmt(len, true),   sub: `(${len.toFixed(2)}초)`, tone: 'ok', warn: len < 1 },
      ].map((c) => (
        <div key={c.label} className="px-7 py-5 flex flex-col items-center text-center">
          ...
          <div className={`num text-[36px] font-semibold tracking-tight leading-none ${
            c.tone==='cyan' ? 'text-brand-cyan' :
            c.tone==='indigo' ? 'text-brand-indigo' :
            c.warn ? 'text-warn' : 'text-ok'
          }`}>{c.big}</div>
          <div className="text-[11.5px] text-fg-mute num mt-2">{c.sub}</div>
        </div>
      ))}
    </div>
  );
...
window.UploadFileCard = UploadFileCard;
window.SelectionInfo = SelectionInfo;
