...
const PageHeader = () => (
  <div className="flex items-start justify-between mb-6">
    <div>
      <h1 className="text-[28px] font-semibold tracking-tight">Key 변환</h1>
      <p className="text-fg-dim text-[14px] mt-1.5">반음 단위로 키를 조절하고 결과를 저장할 수 있습니다.</p>
    </div>
...
const UploadFileCard = ({ state, file = FILE_DEFAULT }) => {
      ...
      <div className="grid" style={{ gridTemplateColumns: '380px 1fr' }}>
        <div className={`relative rounded-xl border ${uploaded ? 'border-line2 border-dashed' : 'border-dashed border-brand-cyan/40 bg-brand-cyan/5'} px-5 py-6 flex flex-col items-center justify-center text-center min-h-[140px]`}>
          <div className={`w-9 h-9 rounded-full grid place-items-center mb-2 ${uploaded ? 'bg-ink-600 text-fg-mute' : 'bg-brand-cyan/15 text-brand-cyan'}`}>
            <Icon name="cloud-up" className="w-4 h-4" />
          </div>
...
window.FILE_DEFAULT = FILE_DEFAULT;
